...
local function runSkillCheck(machine)
    ...
    local inputConnection; inputConnection = UserInputService.InputBegan:Connect(function(input, gp)
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.KeyCode == Enum.KeyCode.Space then
            ...
            task.wait(0.4); skillCheckFrame.Visible = false; successZone.BackgroundColor3 = Color3.fromRGB(255, 255, 255); bar.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
            skillCheckResultEvent:FireServer(currentMachine, wasSuccessful); isGameActive = false
        end
    end)
    tween.Completed:Connect(function() if inputConnection.Connected then inputConnection:Disconnect(); skillCheckFrame.Visible = false; skillCheckResultEvent:FireServer(currentMachine, false); isGameActive = false end end)
    tween:Play()
end
...
local function runMemoryGame(machine, gridSize, pattern)
    ...
    memoryStatus.Text = "Your turn!"; local playerInput = {}; local conns = {}
        ...
        conns[i] = btn.MouseButton1Click:Connect(function()
            if #playerInput < #pattern then btn.BackgroundColor3 = Color3.fromRGB(200,200,100); table.insert(playerInput, i)
                if #playerInput == #pattern then
                    ...
                    else memoryStatus.Text="Failure!"; memoryResultEvent:FireServer(currentMachine, false); task.wait(1); memoryFrame.Visible=false; isGameActive=false end
                end
            end
        end)
    end
...
local function runClassicGame(machine, puzzle)
    ...
    local GRID_SIZE = 6; local grid = {}; for r=1,GRID_SIZE do grid[r]={} end; local nodeObjects = {}
    for _, nodeInfo in ipairs(puzzle) do
        ...
        local nodeData = {label=nodeLabel,r=r,c=c,num=num,color=color,isEndpoint=true}; grid[r][c]={node=nodeData}; table.insert(nodeObjects, nodeData)
    end
    local isDrawing=false; local activePath={}; local activeColor=nil; local pairsConnected=0; local currentPathElements={}
    ...
    local inputBegan; inputBegan = UserInputService.InputBegan:Connect(function(input,gp)
        if gp or not isGameActive or not classicFrame.Visible or input.UserInputType~=Enum.UserInputType.MouseButton1 then return end
        local r,c=getCell(input.Position); if r and grid[r][c] and grid[r][c].node and grid[r][c].node.isEndpoint then isDrawing=true; activeColor=grid[r][c].node.color; table.insert(activePath,{r,c}) end
    end)
    local inputChanged; inputChanged = UserInputService.InputChanged:Connect(function(input,gp)
        if gp or not isDrawing then return end
        local r,c=getCell(input.Position); if r then local lastPos=activePath[#activePath]
            if lastPos and (r~=lastPos[1] or c~=lastPos[2]) and (math.abs(r-lastPos[1])+math.abs(c-lastPos[2])==1) then
                if grid[r][c]==nil then table.insert(activePath,{r,c}); local p=Instance.new("Frame",classicFrame); p.Size=UDim2.new(1/GRID_SIZE,0,1/GRID_SIZE,0); p.Position=UDim2.new((c-1)/GRID_SIZE,0,(r-1)/GRID_SIZE,0); p.BackgroundColor3=BrickColor.new(activeColor).Color; p.ZIndex=2; table.insert(currentPathElements,p)
                elseif #activePath>1 and r==activePath[#activePath-1][1] and c==activePath[#activePath-1][2] then table.remove(activePath); local p=table.remove(currentPathElements); if p then p:Destroy() end
                end
            end
        end
    end)
    local inputEnded; inputEnded = UserInputService.InputEnded:Connect(function(input,gp)
        ...
        if r and grid[r][c] and grid[r][c].node and grid[r][c].node.isEndpoint then
            ...
            if startNode~=endNode and startNode.color==endNode.color then
                ...
                pairsConnected+=1; success=true
                if pairsConnected==6 then classicStatus.Text="Success!"; classicResultEvent:FireServer(currentMachine,true); task.wait(1); classicFrame.Visible=false; isGameActive=false; inputBegan:Disconnect(); inputChanged:Disconnect(); inputEnded:Disconnect() end
            end
        end
        if not success then for _,p in ipairs(currentPathElements) do p:Destroy() end end
        isDrawing=false; activePath={}; activeColor=nil; currentPathElements={}
    end)
end

local function cancelAllGames()
    if not isGameActive then return end; print("Received cancel signal from server.")
...
cancelEvent.OnClientEvent:Connect(cancelAllGames)
